package com.sviat.tictactoe.session.dto;

import com.sviat.tictactoe.session.domain.EngineGameStatus;
import com.sviat.tictactoe.session.domain.PlayerSymbol;
import com.sviat.tictactoe.session.domain.SessionMoveRecord;
import com.sviat.tictactoe.session.domain.SessionStatus;
import java.time.OffsetDateTime;
import java.util.List;

public record SessionResponse(
    String sessionId,
    String gameId,
    SessionStatus sessionStatus,
    EngineGameStatus gameStatus,
    PlayerSymbol nextPlayer,
    PlayerSymbol winner,
    int moveCount,
    PlayerSymbol[][] board,
    List<MoveHistoryItem> moveHistory,
    OffsetDateTime createdAt,
    OffsetDateTime updatedAt
) {
    public record MoveHistoryItem(
        int moveNumber,
        PlayerSymbol player,
        int row,
        int column,
        EngineGameStatus resultingGameStatus,
        OffsetDateTime performedAt
    ) {
        public static MoveHistoryItem from(SessionMoveRecord record) {
            return new MoveHistoryItem(
                record.moveNumber(),
                record.player(),
                record.row(),
                record.column(),
                record.resultingGameStatus(),
                record.performedAt()
            );
        }
    }
}
